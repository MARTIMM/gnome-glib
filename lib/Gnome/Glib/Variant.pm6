...
=item N-GVariantBuilder $builder; a B<GVariantBuilder>
=item N-GVariant $value; a B<N-GVariant>
...
Initialises a B<GVariantBuilder> structure.  I<type> must be non-C<Any>.  It specifies the type of container to construct.  It can be an indefinite type such as C<G_VARIANT_TYPE_ARRAY> or a definite type such as "as" or "(ii)". Maybe, array, tuple, dictionary entry and variant-typed values may be constructed.  After the builder is initialised, values are added using C<g_variant_builder_add_value()> or C<g_variant_builder_add()>.  After all the child values are added, C<g_variant_builder_end()> frees the memory associated with the builder and returns the B<N-GVariant> that was created.  This function completely ignores the previous contents of I<builder>. On one hand this means that it is valid to pass in completely uninitialised memory.  On the other hand, this means that if you are initialising over top of an existing B<GVariantBuilder> you need to first call C<g_variant_builder_clear()> in order to avoid leaking memory.  You must not call C<g_variant_builder_ref()> or C<g_variant_builder_unref()> on a B<GVariantBuilder> that was initialised with this function.  If you ever pass a reference to a B<GVariantBuilder> outside of the control of your own code then you should assume that the person receiving that reference may try to use reference counting; you should use C<g_variant_builder_new()> instead of this function.
...
=item N-GVariant $value; a container B<N-GVariant>
...
Returns: (nullable) (transfer full): a B<N-GVariant>, or C<Any>
